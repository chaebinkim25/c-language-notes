...
3. `make_payment()`를 실행한 결과 문제가 없어서 값이 0이면:
   입장한다.
...
이런 상황을 억지로 `for`문으로 짜면 코드가 어색해진다.
...
시작값도 필요 없고, 규칙적인 갱신도 없으니 자리가 텅 빈다. 여기서 `for` 대신 사용하는 것이 바로 `while`이다.
...
구조는 `if`문과 완전히 똑같다. 단지 `if`는 한 번 실행하고 끝나지만, `while`은 조건이 참인 동안 계속 되돌아가서 반복한다는 점만 다르다.

### 16.3.2 for ↔ while 변환 공식
...
>
> * **for 문:** `continue`를 만나면 **증감식(`i++`)**으로 점프해서 다음 순서를 준비한다. (안전)
> * **while 문:** `continue`를 만나면 **증감식(`i++`)을 건너뛰고** 바로 조건식으로 간다. (증감이 안 되니 **무한 루프**에 빠질 위험 있음!)
>
...
다음 코드를 보자. 0을 만나면 건너뛰고 싶었지만, 결과는 끔찍하다.
...
이를 고치려면 `continue`를 하기 직전에 수동으로 진행(i++)을 시켜줘야 한다.
...
> 코드가 지저분해 보인다면?
>
> 위 코드를 보면 `i++`가 두 번이나 나와서 깔끔하지 않다(중복 코드).
...
확실이 `do-while` 쪽이 코드도 짧고 의도가 명확해진다.

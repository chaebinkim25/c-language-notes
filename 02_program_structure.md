...
C 프로그램은 컴퓨터에게 **무엇을 어떻게 할지 순서대로 적어 놓은 글**이라고 생각하면 된다.  
이 글은 여러 부분으로 나뉘어 있지만, 컴퓨터는 항상 **정해진 한 지점부터** 읽기 시작한다.
...
여기서 `printf`라는 이름은
`<stdio.h>` 안에 미리 정리되어 있다.

즉, `#include <stdio.h>`는 컴퓨터에게 이렇게 말하는 것과 같다.

"이 프로그램에서는 화면 출력과 관련된 기능을 사용할 것이다."
...
중괄호 { }는
**여기부터 여기까지가 하나의 묶음이다**라는 표시다.

컴퓨터는 이 표시를 보고,

어떤 코드들이 함께 실행되는지를 판단한다.

...
프로그램에게 재료란 바로 데이터 _data_ 라고 할 수 있다. 

데이터는 어떻게 구분하고, 어디에 담아서 보관해야 할까?

**재료의 종류: 타입 _type_**

요리 재료에 고기, 채소, 양념이 있듯이 데이터에도 정수, 실수, 문자 같은 종류가 있다.

**재료를 담는 그릇: 변수 _object_**

달걀은 깨지지 않게 달걀 판에 담아야 하고, 우유는 흐르지 않게 컵에 담아야 하듯이, 데이터도 그 종류에 맞는 그릇인 변수에 담아야 한다.

C 언어라는 주방에서 재료를 다루는 법을 본격적으로 배워보자.

## 4.1 프로그램이 하는 일: 데이터를 다루는 3단계 과정

프로그램이 하는 일을 아주 단순하게 요약하면 세 가지 단계로 나뉜다. 
_input-process-output_ 모델이라고 한다.

1. **입력 _input_:** 재료를 준비한다. 키보드 입력, 파일 읽기, 메모리 읽기
2. **처리 _process_:** 요리한다. 계산, 판단, 가공
3. **출력 _output_:** 완성된 요리를 내놓는다. 화면 출력, 파일 저장, 메모리 저장

어떤 복잡한 일을 하든, 모든 과정의 중심에는 항상 데이터가 있다. 아무리 화려하고 복잡한 코드라도, 결국은 데이터를 다루기 위해 존재할 뿐이다.

**데이터가 없다면?**

우리가 매일 듣는 음악 스트리밍 앱을 예로 들어보자.
...
만약 데이터가 없다면, 재생 버튼은 아무런 소리도 낼 수 없고, 화면에는 아무런 노래 제목도 뜨지 않을 것이다. 프로그램은 껍데기일 뿐이다.
...
이 코드를 실행하면 CPU는 내부 회로에서 복잡한 전기 신호를 주고받으며 계산을 수행한다. 하지만 프로그래머인 우리에게 중요한 건, 디지털 회로가 어떻게 연산을 했는가가 아니고, `3`이라는 데이터를 결과값으로 얻었다는 것이다. `3`이라는 결과 값은 다음 코드에서 처리해서 또 새로운 데이터를 만들어낸다. 

프로그래밍이란 필요한 데이터를 메모리라는 창고에 불러오고, 원하는 형태로 가공하고, 다시 저장하는 과정의 연속이라고 할 수 있다.

## 4.2 코드에 직접 적혀있는 데이터: 리터럴 _literal_
...
소스 코드에 직접 등장하는 값을 리터럴 _literal_ 이라고 부른다.

**왜 리터럴인가?**

_literal_ 은 영어로 문자 그대로라는 뜻이다.
메모리에 저장되는 데이터는 상황에 따라 값이 `10`이 될 수도 있고 `100`이 될 수도 있지만, 리터럴 `0`은 말 그대로 언제나 `0`이다.
...
- **변하지 않는다:** 프로그램이 실행되는 동안 `0`이 `1`로 변하는 일은 절대 없다.
...
> - `1`, `2`: 이것들은 리터럴이다.
>
> 리터럴 `1`, `2`는 수식 `1 + 2`를 만드는데 쓸 수 있다.
...
프로그래밍에서는 이렇게 생성된 구체적인 데이터의 실체를 값 _value_ 라고 부른다. 
...
C 언어는 생성-저장-사용-소멸 과정을 명확하게 드러내서 프로그래머가 직접 다루도록 설계되어 있다.

## 4.4 값으로 계산을 한다는 것: 연산 _operation_

"컴퓨터는 계산기니까 수학을 엄청 잘하겠지?"

반은 맞고 반은 틀린 말이다. 
컴퓨터가 수행하는 연산 _operation_ 은 우리가 배우는 수학처럼 복잡해 보이지 않는다.
...
- **산술 연산 _arithmetic_:** 숫자를 더하거나 빼서 새로운 값을 만든다.

  `10 + 20`으로 `30`이라는 새로운 숫자 값을 만든다.

- **대입 연산 _assignment_:** 값을 저장한다.
...
**계산의 도구: 연산자 _operator_**

요리를 하려면 칼이나 불 같은 도구가 필요하듯, 데이터를 지지고 볶으려면 도구가 필요하다.
연산을 하게 해주는 기호들을 연산자 _operator_ 라고 부른다.

- **`+`, `-`, `*`, `/`, `%`:** 더하고, 빼고, 곱하고, 나누고, 나머지를 구하는 도구
- **`=`:** 데이터를 상자에 담는 도구

**계산의 순서: 우선순위 _precedence_**

수학에서 $1 + 2 \times 3$을 계산할 때 곱셈을 먼저 하듯, C 언어에도 순서가 있다.
컴퓨터에서는 정해진 우선순위에 따라 연산자들을 하나씩 처리한다.
우선순위는 괄호 `()`가 제일 높고, 곱셈류 산술 연산자 `*`, `/`, `%`가 그 다음이고, 
덧셈류 산술 연산자 `+`, `-`가 그 다음이고, 대입연산자 `=`가 제일 낮다. 

**계산의 방향: 결합성 _associativity_**

우선순위가 같은 연산자들이 나열되면 결합 방향에 따라서 계산 순서가 정해진다. 
산술 연산자는 왼쪽에서 오른쪽으로 계산되고, 대입연산자는 오른쪽에서 왼쪽으로 계산된다. 

**계산의 과정: 수식 _expression_**

수학에서처럼 연산자와 데이터가 뭉쳐서 하나의 결과값을 만들어내는 단위를 수식 _expression_ 이라고 부른다.
...
프로그래밍에서는 데이터를 담기 위해 메모리 안에 특정 공간을 배정받는데, 이 공간을 변수 _object_ 라고 부른다. 
...
- **변수:** 값을 담는 상자인 메모리 공간
- **식별자:** 상자에 붙인 이름표인 변수 이름
- **값:** 상자에 들어있는 내용물인 데이터

## 4.6 0과 1에 의미를 부여하는 안경: 타입 _type_
...
C 언어에서 타입 _type_ 이란, 메모리에 담긴 `0`과 `1`을 어떻게 해석할지 결정하는 규칙이다. 

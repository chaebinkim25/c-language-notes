...
## 13.9 구조체를 동적으로 만들기: `malloc`

*"5각형, 100각형은 어떻게 만들지?"* 라는 질문에 대한 해답이다.
동적 할당(Dynamic Allocation)을 구조체에 적용하면, 실행 중(Run-time)에 우리가 원하는 크기만큼 구조체를 찍어낼 수 있다.

구조체도 `int`나 `double`과 똑같다. `sizeof`로 크기만 구해서 `malloc`에 넘겨주면 메모리를 빌릴 수 있다.

### 13.9.1 구조체 한 개 동적 할당

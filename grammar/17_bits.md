...
* **동작:** 비트들을 오른쪽으로 n칸 민다.
* **수학적 의미:** **2의 n승으로 나누는 효과** ($x / 2^n$) (몫만 남음)

**① 핵심 용도: "상위 비트 정보 추출하기"**
높은 자리에 있는 데이터를 0번 자리(1의 자리)로 끌고 내려와서 읽을 때 사용한다.
예를 들어, 8비트 데이터 중 상위 4비트에 "레벨" 정보가 있다고 가정해보자.

		1 1 0 1 0 0 0 0  (원본: 상위 4비트가 중요함)
		>> 4             (오른쪽으로 4칸 당기기)
		---------------
		0 0 0 0 1 1 0 1  (결과: 13이라는 값으로 읽을 수 있음)

**② 주의: "음수의 빈자리는 누가 채우나?"**
오른쪽으로 밀면 왼쪽 끝(부호 비트 쪽)에 빈자리가 생긴다. 여기에 `0`이 들어올까 `1`이 들어올까?
여기서 **논리 시프트**와 **산술 시프트**의 개념이 갈린다.

1.  **논리 시프트 (Logical Shift):** 빈자리를 무조건 **`0`**으로 채운다.
    * 주로 **`unsigned`** 타입에서 사용된다. (데이터의 모양 유지)
2.  **산술 시프트 (Arithmetic Shift):** 빈자리를 **부호 비트와 같은 값(`0` 또는 `1`)**으로 채운다.
    * 주로 **`signed`** 타입에서 사용된다. (음수의 부호를 유지하기 위함)

**[문제점: 구현 정의 (Implementation Defined)]**
C 언어 표준은 `signed` 타입의 오른쪽 시프트에 대해 **"구현 정의"**라고 명시했다. 즉, **컴파일러 마음대로**라는 뜻이다.

* 대부분의 현대 컴파일러는 음수를 유지하려고 `1`을 채우지만(산술), 어떤 컴파일러는 그냥 `0`을 채울 수도 있다(논리).
* 만약 음수 `-4`(`...11100`)를 밀었는데 `0`이 들어오면, 갑자기 거대한 양수로 변해버린다.

> **🚨 결론:**
> "나눗셈을 하려는 게 아니라 비트를 조작하려는 것"이라면, **무조건 `unsigned` 형변환 후 시프트**를 해야 안전하다.

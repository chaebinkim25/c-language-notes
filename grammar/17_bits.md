...
우리가 쓰는 `int`는 대부분의 환경에서 32비트(4바이트) 크기다.
그런데 'ON 상태인가?'를 저장하는 데는 사실 단 1비트(0 또는 1)면 충분하다.
...
```
		1u << n
		(숫자 1u의 비트를 왼쪽으로 n칸 밀어라)
```

이 문법 하나면 모든 위치의 마스크를 즉석에서 만들 수 있다.

- **`1u << 0`** : `000...0001` (0번 마스크)
- **`1u << 1`** : `000...0010` (1번 마스크)
- **`1u << 5`** : `000...00100000` (5번 마스크)
...
> [!TIP]
> 이건 원리를 보여주는 장난감 예시다.
> 키를 재사용하면 쉽게 깨질 수 있으니 보안 목적 암호로는 쓰지 말자.
...
> **팁: `~0u`는 무엇일까?**
>
> 프로그래밍을 하다 보면 `~0u`라는 코드를 종종 보게 된다.
> `0u`는 모든 비트가 `0`인 상태다. 그걸 뒤집으면? 모든 비트가 1인 상태가 된다.
>
...
`1u << n`을 하면, n번째 비트만 1이고 나머지 비트는 0이 된다. 

- "3번 스위치를 켜는 마스크가 필요해?" `1u << 3`
- "5번 스위치를 켜는 마스크가 필요해?" `1u << 5`
...
> 
> ```c
>		unsigned int u = 0x80000000u; /* (1000...0000) */
>		u = u << 1;                   /* (0000...0000) - 맨 앞의 1이 버려짐 */
>		/* u에 저장된 값: 0 (문제없음, 의도된 동작) */
> ```
>
> 단, $n$이 음수이거나 비트 폭 이상이면 `unsigned`라도 미정의 동작이다.
> 
...
## 17.9 복합 대입 연산자: 비트 연산도 줄여 쓰자 (`|=`, `&=`, `^=`)

우리는 16장에서 변수의 값을 갱신할 때 코드를 줄여 쓰는 복합 대입 연산자를 배웠다.
`i = i + 1`을 `i += 1`로 줄여 쓸 수 있었던 것처럼, 비트 연산자도 똑같은 문법을 지원한다.

비트 연산은 자기 자신을 갱신하는 경우가 굉장히 많기 때문에, 실무 코드에서는 풀어 쓰는 방식보다 이 복합 대입 방식을 표준 관용구처럼 사용한다.

**켜기 (`|=`)**

기존 플래그에 새로운 속성을 추가할 때 가장 많이 사용한다.

* **긴 코드:** `flags = flags | MASK;`
* **줄인 코드:** `flags |= MASK;`

**끄기 (`&=`)**

특정 속성만 제거할 때 사용한다. (반전된 마스크와 AND 연산)
...
> **읽는 법**
> 
> `flags |= MASK`를 소리 내어 읽을 때는 "flags에 MASK를 OR하여 대입한다" 또는 더 직관적으로 "flags에 MASK 속성을 추가한다"고 읽으면 코드를 이해하기 훨씬 쉽다.

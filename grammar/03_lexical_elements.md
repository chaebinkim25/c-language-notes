...
구두점은 연산자와 달리 값을 계산 하지 않고, 코드를 묶거나 나누는 구조족인 역할만 한다.

### 3.3.5 연산자(operator): 계산을 위한 기호

마지막으로 덧셈(`+`), 뺄셈(`-`) 같은 기호들도 토큰의 한 종류로 연산자(operator)라고 부른다. 연산자는 값을 계산하거나 비교하는 동작을 수행한다. 지금까지의 예제에는 연산자가 거의 등장하지 않았지만, 이후 장에서 자세히 다룬다.

...
영어와 비교해보면, 문단 _block_ 을 나누고 문장 _statement_ 을 왼쪽 위에서부터 읽어 내려가는 '읽기 규칙'을 배운 셈이다. 

다음 순서는 단어를 배울 차례다.

영어 문장을 이해하려면 알파벳이 모여 만든 단어의 뜻을 알아야 하듯, C 언어도 코드를 구성하는 단어 하나하나를 알아야 비로소 해석할 수 있다.

## 3.1 C 코드의 글자들에 의미를 부여하는 단위: 토큰 _token_

컴파일러는 소스 코드를 먼저 여러 조각으로 나눈 뒤에 의미를 해석한다. 그 과정에서 더 이상 나누지 않고 하나의 의미로 다루는 단위를 토큰 _token_ 이라고 부른다.
...
또 다른 주목할 점은 단어 `int`, `main`, `void`, `return`, `0` 뿐만 아니라 기호 `(`, `)`, `{`, `}`, `;` 하나하나도 모두 독립적인 토큰으로 취급된다는 것이다.

또한, 영어에 명사, 고유 명사, 동사 같은 품사가 있듯이 토큰에도 종류가 있다. 다음 절들에서 토큰의 종류를 하나씩 알아본다. 

## 3.2 C 코드를 토큰으로 나누는 규칙: 가장 긴 단어로 _maximal munch_
...
- **띄어쓰기:** 공백이 있으면 단어가 끝난다. `코드를`과 `코딩하다` 사이의 띄어쓰기가 단어를 구분한다.
- **문장부호:** 기호는 글자와 붙어 있어도 별개의 단어로 친다. `코딩하다`와 `?`는 별개다.
...
- **공백 규칙:** 
  ...
  띄어쓰기, 탭, 줄바꿈과 같은 공백은 토큰을 분리하는 가장 명확한 경계다.
...
- **숫자 규칙:**
  ...
  - 만약, 문자 뒤에 숫자가 붙어있었다면, `return0`처럼 하나의 토큰이 될 수 있다.
...
> 기호 두개로 이뤄진 토큰들은 연산자들을 표현하기 위해 쓰인다.

## 3.3 토큰의 종류: 역할에 따라 나뉘는 다섯 가지

C 언어라는 세계를 구성하는 토큰은 역할에 따라 크게 키워드, 식별자, 리터럴, 구두점, 연산자의 5가지로 나뉜다.
하나씩 자세히 알아가보자. 

### 3.3.1 C 언어 차원에서 미리 예약해둔 단어: 키워드 _keyword_

우리말에서 사전에 뜻이 적혀 있는 단어는 '학교'처럼 그 단어를 들으면 누구나 떠올리는 정해진 의미가 있다. C 언어 세상에서도 특별한 용도로 쓰기로 약속된 단어들이 있다.

프로그래밍 세상에서는 이러한 단어를 키워드 _keyword_ 또는 예약어 _reserved word_ 라고 부른다.
...
### 3.3.2 프로그래머가 지어주는 이름: 식별자 _identifier_

'수원', '화성'처럼 대상을 구분하기 위해 붙여준 고유한 이름을 프로그래밍에서는 식별자 _identifier_ 라고 한다.
키워드가 이미 정해진 단어라면, 식별자는 프로그래머가 필요에 따라 직접 지어주는 이름이다.

간단한 C 프로그램에서도 식별자가 있었다.

- **`main`:** 프로그램의 시작 함수를 가리키는 이름으로 C 언어의 약속된 식별자다.

`main`은 식별자 중에서 가장 특별한데, 일반적인 환경에서 실행되는 C 프로그램은 `main`이라는 이름의 함수에서 시작한다.
...
> **용어 정리: 리터럴과 상수**
>
> C 표준 문서에서는 리터럴 대신 상수 _constant_ 라는 용어를 써서 정수 상수, 문자 상수와 같이 부른다.
> 하지만 상수는 값을 담고 있지만 한 번 정해지면 바꿀 수 없는 데이터 공간이라는 의미도 담고 있기 때문에,
> 여기서는 명확한 표현을 위해 리터럴이라는 용어를 사용한다. 

### 3.3.4 문장의 구분 기호: 구두점 _punctuator_

우리말에서 단어만 나열하면 문장을 이해하기 어렵기 때문에, 글을 쓸 때 마침표 `.`나 쉼표 `,`가 필요하다. 
C 코드에서도 문장의 끝을 알리거나 구조를 묶어주는 기호들이 필요하며, 이러한 기호를 구두점 _punctuator_ 이라고 한다.

간단한 C 프로그램에도 구두점이 있었다. 

- **`;` 세미콜론:** 문장의 마침표 역할. "여기서 명령이 끝났다"
- **`{ }` 중괄호:** 여러 문장을 하나로 묶는 울타리 역할
- **`( )` 소괄호:** 함수와 관련된 부분을 표시하거나, 연산 순서를 명확히 하는 데 사용된다.

구두점은 연산자와 달리 값을 계산 하지 않고, 코드를 묶거나 나누는 구조적인 역할만 한다.

### 3.3.5 계산을 위한 기호: 연산자 _operator_

마지막으로 덧셈 `+`, 뺄셈 `-` 같은 기호들도 토큰의 한 종류로 연산자 _operator_ 라고 부른다. 연산자는 값을 계산하거나 비교하는 동작을 수행한다. 지금까지의 예제에는 연산자가 거의 등장하지 않았지만, 5장에서 자세히 다룬다.
        
## 3.4 투명한 글자: 공백 _whitespace_
...
프로그래밍에서는 이들을 통틀어 공백 _whitespace_ 라고 부른다. 종이의 여백처럼 보이기 때문이다.
...
하지만 공백이 데이터의 일부로 취급되거나 구두점 역할을 하는 곳이 있다. 첫번째는 문자열 여러개를 소스 코드에 적을 때다. 그 때는 공백도 문자들 중 하나로 취급한다. 

두번째로는 텍스트 전처리 문법에서이다. 전처리 문법에서는 기본적으로 줄바꿈이 한 단위의 끝을 나타낸다. 

**우리는 왜 공백을 쓸까?**

대부분의 경우, 컴퓨터가 신경 쓰지 않는데도 우리가 들여쓰기를 해서 줄을 맞추고, 적절히 띄어 쓰기를 하는 이유는 오직 사람을 위해서다.
나중에 코드를 다시 볼 '미래의 나'와 '동료 개발자'가 코드를 쉽게 읽을 수 있도록 하는 배려, 이것이 바로 가독성 _readability_ 이다.
가독성은 프로그램의 기능에는 영향을 주지 않지만, 유지보수에는 큰 영향을 준다.
읽기 쉬운 코드는 오류를 줄인다. 

> [!TIP]
> **커뮤니티의 문화: 코딩 스타일 _coding style_**
>
> 개발자들은 공백을 어떻게 쓸지 약속을 정해두곤 하는데, 이런 세세한 규칙들을 통틀어 '코딩 스타일'이라고 부른다.

...
C 언어에는 숫자를 다루는 여러 가지 방법이 있지만, 우리는 가장 기본이 되는 정수 타입 `int`부터 다룬다. 

## 5.0 이 장의 목표: `int` 변수 선언과 산술 계산

이 장의 목표는 명확하다. 
6장의 조건 문장, 그리고 7장의 반복 문장으로 넘어가기 위한 기초 체력을 다지는 것이다.

- **`int` 타입 변수 선언과 초기화:** 선언을 통해 정수를 담을 메모리 공간을 확보하고, 초기화를 통해 처음 저장할 값을 정한다.

- **산술 계산하기:** 더하고 `+`, 빼고 `-`, 곱하고 `*`, 나누는 `/` 법을 배운다. 나머지 연산자 `%`의 의미와 활용을 이해한다.
...
## 5.1 왜 하필 `int`부터 시작하나: 프로그램 흐름의 지휘봉
...
- **세기 _counting_:** "하나, 둘, 셋..." 숫자를 센다. `count + 1`
- **반복 _iteration_:** "이 작업을 10번만 해라." `i < 10`
- **순서 _indexing_:** "첫 번째 데이터..." 데이터에 번호표를 붙인다. `data[0]`
- **분기 _condition_:** "값이 0보다 크면 실행해라." `value > 0`
- **상태 _state_:** "성공이면 0을 반환해라." `return 0;`

이 작업들의 공통점은 대부분 딱 떨어지는 정수의 계산과 비교로 이루어진다는 점이다.

소수점이 있는 실수 `3.14`나 문자열 `"Hello"`는 데이터의 내용을 담당하지만, 프로그램이 어디로 갈지, 몇 번 반복할지, 언제 멈출지를 결정하는 핵심 도구는 `int`다.
...
> [!NOTE]
> 
> **보안 관점: `int`와 재전송 공격 방어**
>
> 보안 분야에서도 `int`는 필수적이다.
> 해커가 똑같은 데이터를 몰래 다시 보내는 재전송 공격 _replay attack_ 을 막으려면 어떻게 해야 할까?
> 가장 쉬운 방법은 데이터마다 1, 2, 3... 처럼 정수 번호표를 붙이는 것이다.
> 전에 10번 데이터를 받았는데, 또 10번이 들어오면 "아, 이건 가짜구나!" 하고 버릴 수 있다.
> 정수 하나가 시스템의 보안을 지키는 핵심 열쇠가 되기도 한다.

## 5.2 `int`란 무엇인가: C 언어의 기본 정수 타입

`int`는 정수 _integer_ 의 줄임말로, C 언어에서 가장 기본이 되는 데이터 타입이다.

**정수 규칙을 따른다.**

`int`는 소수점이 없는 정수만을 다룬다.

가장 큰 특징은 나눗셈 결과에 수학적으로 소수 부분이 생기더라도 이를 잘라낸다는 점이다. 여기서 자른다는 것의 의미는 0을 향해 자른다는 것이다 _truncate toward zero_.

```c
5 / 2;    /* 결과: 2.5에서 .5를 버리면 2가 된다 */
-5 / 2;   /* 결과: -2.5에서 .5를 버리면 -2.5가 된다 */
```
...
> 하지만 C99 표준 이후로는 `0`을 향해 자른다고 정의되었다.
...
> **넘침 현상 _overflow_**
>
> `int`가 담을 수 있는 한계를 넘으면 오버플로우 _overflow_ 가 발생한다.
> 그 결과 예측할 수 없는 오류가 발생하므로 주의해야 한다.

> [!TIP]
> 
> **유연한 습관 기르기**
>
> C 언어는 슈퍼컴퓨터부터 아주 작은 임베디드 센서까지 다양한 컴퓨터에서 동작하도록 설계되었다.
> 따라서 `int`는 항상 4바이트라고 가정하기보다는,
> 시스템이 정한 기본 크기를 따른다고 유연하게 생각하는 습관이 중요하다.

## 5.3 `int` 값을 코드에 적는 방법: 정수 리터럴 _literal_
코드에 값 자체를 직접 적은 것을 리터럴 _literal_ 이라고 부른다. C 언어에서 정수 리터럴은 우리가 아는 숫자를 쓰는 방식과 같이 적으면 된다.
...
## 5.4 `int` 변수 _object_: 이름 붙인 `int` 상자

이전에 데이터를 상자에 담고 이름을 붙인다고 했다. 
C 언어에서 그 이름 붙은 상자가 바로 변수 _object_ 다.

### 5.4.1 변수를 만드는 법: 선언 _declaration_
...
- **타입 `int`:** 이 상자는 `int` 타입으로 선언되어 해당하는 타입에 맞는 크기의 메모리가 할당된다. 저장된 값은 정수 규칙에 따라 해석되고 계산된다. 
- **이름 `x`:** 이 상자의 이름표는 `x`다.
...
> [!TIP]
> 
> **변수 이름 짓기 규칙 _naming rule_**
>
> - 영어 대소문자 `a-z`, `A-Z`, 숫자 `0-9`, 밑줄 `_` 만 쓴다. C89 기준 한글 ❌, 공백 ❌, 특수문자 ❌
> - **숫자로 시작할 수 없다:** `1st_player` ❌, `player1` ⭕
> - **예약어는 사용할 수 없다:** `int` ❌, `return` ❌
> 
> **Q. 최신 C 언어는 한글 변수 이름도 된다던데?**
>
> A. 최신 표준 C23부터 가능하지만, 일부 환경에서는 정상적으로 표시되지 않거나, 호환성 문제가 발생할 수 있어서 C 언어로 프로그래밍을 할 때는 영어를 쓰자.

### 5.4.2 선언 위치: C99 표준 이후는 블록 안, C89 표준은 무조건 맨 위에!

C89 표준에는 다음과 같은 규칙이 있다. 
변수 선언은 반드시 `{ }` 블록의 맨 처음에 몰아서 해야 한다.

요리를 시작하기 전에 필요한 재료인 변수를 조리대 위에 다 꺼내놓고 시작하는 방식이다.
...
### 5.4.3 선언과 동시에 값을 저장하는 법: 초기화 _initialization_

변수를 만들면서 동시에 값을 저장하는 초기화 _initialization_ 문법이 있다.
...
> [!CAUTION]
> 
> **쓰레기 값 _garbage value_ 주의보**
>
> 초기화를 하지 않고 `int x;`만 하면, 상자 안에 무엇이 들어있을까?
> `0`이 들어있을 것 같지만, 실제로는 알 수 없는 값이 들어있다.
> 
> 왜 그럴까?
> 
...
>
> 알 수 없는 값을 읽으면 정의되지 않은 동작 _undefined behavior_ 가 발생한다. 
> 변수를 만들 때는 가능하면 초기화를 하거나, 사용하기 직전에 반드시 값을 대입해야 한다.

## 5.5 `int` 값을 바꾸는 가장 기본 동작: 대입 _assignment_

변수를 만들었으니, 이제 그 안에 값을 넣거나 바꿀 차례다.

### 5.5.1 값을 저장하는 연산자: 대입 연산자 _assignment operator_ `=`
...
- **`==` 같은지 비교:** 양쪽이 서로 같은지 비교하여 참 `1` 또는 거짓 `0`을 결과값으로 낸다.

**`=`의 왼쪽과 오른쪽의 엄격한 역할: _lvalue_ 와 _rvalue_**

대입 연산자 `=`는 왼쪽과 오른쪽의 역할이 다르게 정해져 있다.

- **왼쪽 피연산자:** 값을 저장할 수 있는 대상만 올 수 있다. _lvalue_ 라고 하며 left value에서 유래했다. 
- **오른쪽 피연산자:** 어떤 값이든 올 수 있다 _rvalue_ 라고 하며 right value에서 유래했다.
...
**대입 연산의 결과: 덮어쓰기 _overwrite_**

위 코드에서 `x = 10;`을 실행한 뒤 `x = 20;`을 실행하면 어떻게 될까?

변수 상자는 한 번에 하나의 값만 담을 수 있다. 
새로운 값 `20`이 들어오는 순간, 기존에 있던 값인 `10`은 더 이상 저장되어 있지 않다. 
...
### 5.5.2 `=`의 결합 방향 _associativity_: 오른쪽에서 왼쪽으로
...
대입 연산자 `=`의 특징은 흐름이 오른쪽에서 왼쪽 _right associative_ 라는 점이다. 
...
### 5.5.3 `=`의 우선순위 _precedence_: 거의 제일 낮다
...
### 5.5.4 컴퓨터가 숫자를 세는 방법: `x = x + 1`

변수의 이름과 같이 데이터가 저장된 상자를 가리키는 수식을 _lvalue_ 라고 부른다.
_lvalue_ 는 저장 가능한 대상을 의미하며, 값이 필요한 맥락에서는 저장된 값으로 평가된다. 
...
1. **읽기 _rvalue_:** 오른쪽의 `x` 상자에서 현재 값 `1`을 꺼낸다.

2. **계산:** 꺼낸 값에 `1`을 더해 `2`를 만든다.

3. **쓰기 _lvalue_:** 결과값 `2`를 왼쪽의 `x` 상자에 덮어쓴다.

비슷한 패턴이 앞으로 숫자를 세거나, 합계를 구할 때 많이 사용될 것이다. 

> [!NOTE]
> 
> **보안 관점: 순서 번호 _sequence number_**
>
> 앞서 재전송 공격을 막기 위해 데이터마다 번호표 `count`를 1씩 증가시키기도 한다고 했다.
> 번호표를 1씩 증가시키는 방법이 바로 `count = count + 1`을 하는 것이다.

> [!CAUTION]
> 
> **위험! 두 마리 토끼를 쫓지 마세요 (*undefined behavior*)**
>
> C 언어에서는 실행 순서가 정해져 있지 않은 상황에서 같은 변수를 두 번 수정하거나, 수정과 읽기를 함께 하면 안 된다.
> 만약 이를 어기면 그 결과는 표준에서 보장되지 않으며, 정의되지 않은 동작 _undefined behavior_ 가 된다 .
>
> ```c
> int x = 0, y = 0;
> y = x + (x = 1);    /* 위험! */
...
> 아니면 저장하기 전에 먼저 읽어서 `0`으로 읽을지는 표준에 정해져 있지 않다 _unsequenced_.
> 
...
>
> 대입 연산 `=`에서 값을 저장하는 부수 효과 _side effect_ 는,
> 피연산자들의 값 계산이 끝난 이후 _sequenced after_ 에 발생한다. 
>
> `x = x + 1`에 이 규칙을 적용해보면,
> 
> 1. **값 계산:**
...
>    -  계산된 값을 `x`에 저장한다.
>
> `x`에 대한 읽기와 쓰기의 실행이 분리되어 있기 때문에, `x = x + 1`은 언제나 안전하다.
...
### 5.6.1 피연산자 _operand_: 연산자의 의미를 정하다

연산자가 어떻게 계산을 수행할지와 결과값이 어떤 타입일지는 연산을 할 값들인 피연산자 _operand_ 의 타입이 결정한다.
...
이때 버리는 방향이 중요한데, C 언어는 무조건 0을 향해 버린다 _truncate toward zero_.
...
> 컴파일러에 따라서 `-10 / 3`이 `-3`이 될 수도 있고, `-4`가 될 수도 있었다.
> 하지만 C99 표준부터는 소수 부분을 무조건 0 쪽으로 버리도록 통일되었다.

### 5.6.3 `int`의 나머지 `%`: 왼쪽의 부호를 따라간다

`%` 연산자는 나눗셈의 짝꿍이다. 그런데 음수가 섞이면 나머지의 부호가 헷갈린다.
규칙은 간단하다. 나머지의 부호는 나누어지는 수인 왼쪽 피연산자의 부호를 따라간다.
...
> $$a = (a / b) \times b + (a \% b)$$
> 몫이 0 쪽으로 절삭되므로(`-3.33`이 `-3`으로), 나머지는 자연스럽게 왼쪽 수(`a`)의 부호를 따라가게 된다.
...
> 
> `-10 % 3`이 `-1`이 될 수도 있고, `2`가 될 수도 있었다.
...
```c
int x = 2 + 3 * 4;
/* 1. 곱셈 3 * 4를 먼저 해서 12가 됨
   2. 덧셈 2 + 12를 해서 14가 저장됨 */
```
...
### 5.6.6 피연산자가 하나인 연산자: 단항 연산자 _unary operator_ `+`, `-`

지금까지 본 더하기 `+`, 빼기 `-`는 양옆에 숫자가 2개 필요한 이항 연산자 _binary operator_ 였다.
하지만 `+`와 `-` 기호는 숫자 하나에만 붙어서 작동하기도 하는데, 이를 단항 연산자 _unary operator_ 라고 부른다.

- **이항 _binary_:** `10 - 3` 숫자 2개 필요, "빼기"
- **단항 _unary_:** `-3` 숫자 1개 필요, "부호 바꾸기"

| 기호 | 이름 | 역할 | 예시 (`a=10`) | 결과 |
| :---: | :---: | :--- | :--- | :---: |
| **`-`** | **부호 반전 연산자** | 부호를 뒤집는다. 양수와 음수를 바꾼다. | `-a` | `-10` |
| **`+`** | **부호 유지 연산자** | 아무 일도 안 한다. 강조용이다 | `+a` | `10` |
...
>
> C 언어 컴파일러에는 가능한 긴 연산자를 먼저 만든다는 _maximal munch_ 규칙이 있다.
...
> ```
>
> - **첫 번째 `-`:** 양옆에 숫자가 있으므로 이항 연산자인 빼기
> - **두 번째 `-`:** 뒤에만 숫자가 있으므로 단항 연산자인 부호 반전
> - **해석:** `10 - (-5)` → `10 + 5` → `15`

### 5.6.7 우선 순위 총정리: 괄호 다음 단항 연산자
...
## 5.7 아주 중요한 금지 사항 2개: 0으로 나누기와 오버플로우

정수 계산을 할 때, 문법적으로는 틀리지 않았지만 실행하는 순간 프로그램을 망가뜨리는 두 가지 치명적인 함정이 있다.

### 5.7.1 불가능한 계산: 0으로 나누기 _division by zero_

수학에서 0으로 나누는 것이 정의되지 않듯이, 컴퓨터에서도 `0`으로 나누기는 금기다.
...
실수로라도 나누는 수 분모가 `0`이 되면, 프로그램은 그 즉시 비정상 종료 _crash_ 되거나 예측할 수 없는 상태에 빠진다. 컴파일러가 미리 경고해 줄 때도 있지만, 변수에 `0`이 들어있는지 컴파일러가 모를 때가 더 많으므로 프로그래머가 항상 주의해야 한다.

### 5.7.2 불가능한 값: `int` 범위를 넘는 계산 _overflow_
...
이것을 오버플로우 _overflow_ 라고 부른다. 컵에 물을 계속 부으면 넘쳐흐르듯이, 변수가 감당할 수 있는 범위를 넘어서면 값이 깨져버린다. 아주 큰 수는 나중에 배울 `long long`등을 써야 한다. 

...
> 해커가 똑같은 데이터를 몰래 다시 보내는 재전송 공격(*replay attack*)을 막으려면 어떻게 해야 할까?
> 가장 쉬운 방법은 데이터마다 1, 2, 3... 처럼 번호표(정수)를 붙이는 것이다.
> 전에 10번 데이터를 받았는데, 또 10번이 들어오면 "아, 이건 가짜구나!" 하고 버릴 수 있다.
> 고작 정수 하나가 시스템의 보안을 지키는 핵심 열쇠가 되기도 한다.
...
>
> A. 최신 표준(C23)부터 가능하지만, 일부 환경에서는 정상적으로 표시되지 않거나, 호환성 문제가 발생할 수 있어서 C 언어로 프로그래밍을 할 때는 영어를 쓰자.
...
이때 버리는 방향이 중요한데, C 언어는 무조건 0을 향해 버린다 (*truncate toward zero*).
...
>
> C 언어 컴파일러에는 가능한 긴 연산자를 먼저 만든다는 규칙이 있다 (*maximal munch*).

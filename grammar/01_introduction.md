...
- **성능 우선:** 실행 중 *runtime*에 "이게 맞아?"라고 검사하는 과정을 생략하여 속도를 극대화한다.
...
4. **미정의 동작 *undefined behavior, ub***
   ...
   - **예:** 허가받지 않은 메모리에 대한 접근 *buffer overflow*.
...
4. **성능우선주의와 보안의 트레이드오프:** 배열 인덱스
   ...
   - **보안 위험 *buffer overflow*:** 프로그래머가 실수로 배열의 크기를 넘어서도, 예를 들면, 크기 100인 배열 `arr`에서 `arr[100]`, 컴퓨터는 경고 없이 메모리의 다른 중요한 값을 덮어써 버린다. 그래서 해킹의 주요 통로가 된다. 

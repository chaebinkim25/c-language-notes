...
> 차체(언어)가 가벼워져서 속도는 엄청나게 빠르지만, 운전(프로그래밍)을 잘못하면 가드레일을 들이받을 수도 있다.
...
3. **미지정 동작 (Unspecified Behavior)**
   - **의미:** 여러가지 올바른 방법 중 C 언어 번역기가 그때그때 가장 효율적인 것을 선택하도록 놔둔 동작이다.

...
이번 장에서 우리는 주소를 꺼내고(&), 주소를 담고(int*), 그 주소로 가서 값을 바꾸는(*) 방법을 배웠다.
그리고 포인터를 한 칸씩 움직이며 배열을 훑는 패턴까지 손에 넣었다.
다음 장에서는 이 기술을 문자열에 적용해 본다. C의 문자열은 특별한 마법이 아니라, char들이 줄지어 있는 배열이며, 끝을 알리는 표식를 만날 때까지 스캔하는 데이터다.

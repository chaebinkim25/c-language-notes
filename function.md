...
### 9.10.2 왜 선언이 좋은가 (안전장치)

선언을 해두면 좋은 점이 있다.

- 컴파일러가 인자 개수가 맞는지 검사할 수 있다.
- 각 인자가 매개변수 타입에 들어갈 수 있는지(대입 가능한지)도 검사할 수 있다. 

예를 들어, 이런 실수는 선언이 없으면 더 위험해진다.

```c
sum = sum_array(a, 5, 123);  /* 인자 3개를 넣어버림 (실수) */
```

함수 선언(원형)을 해 두면 컴파일러가 "너 지금 약속이랑 다르게 호출했어"라고 잡아준다.

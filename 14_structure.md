...
struct Contact {
    ...
    char phone[12];    /* 배열도 멤버로 포함 */
};

int main(void) {
    /* 1. 1번 연락처 정보 생성 */
    struct Contact c1 = { "Alice", "01012345678" };
    struct Contact c2;

    /* 2. 대입 연산: c1의 모든 내용이 c2로 복사됨 */
    c2 = c1; /* (O) OK! */

    /* c2에는 { "Alice", "01012345678" }가 저장되어 있다. */
    
    return 0;
}
```

여기서 `name`은 배열로 분명 대입이 안되지만, `c2 = c1`을 실행하는 순간 `c1.name`의 문자열들이 `c2.name`으로 완벽하게 복사된다.
...
## 14.8 태그 없는 구조체 (Anonymous Structure)
...
## 14.10 구조체를 동적으로 만들기: `malloc`

앞 절(14.9)에서 제기했던 *"5각형, 100각형은 어떻게 만들지?"* 라는 질문에 대한 해답이다.
...
if (shape == NULL) return -1; /* 메모리 부족 예외 처리 */
...
## 14.11 구조체 안에 포인터가 있을 때: 얕은 복사의 함정

14.10에서 만든 `struct Polygon`을 다시 보자. 
...
### 14.11.1 얕은 복사(Shallow Copy)가 뭐가 문제인가?
...
### 14.11.2 해결책: "깊은 복사(Deep Copy)" 규칙 만들기
...
```
/* 깊은 복사를 수행하는 함수 */
int copy_polygon(struct Polygon *dest, const struct Polygon *src) {
    int i;
...
## 14.13 이름 없이 즉석에서 만들기: 복합 리터럴 (C99)
...
### 14.13.3 가독성 높이기: 지정 초기화자와 섞어 쓰기
